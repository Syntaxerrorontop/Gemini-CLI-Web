...
import { Play, Send, Terminal, Settings, CheckCircle2, Circle, History, Clock, RotateCcw } from 'lucide-react';
...
export default function ProjectView() {
  ...
  const toggleStep = async (stepIdx: number) => {
    ...
    const newStep = stepIdx === current + 1 ? stepIdx : stepIdx - 1;
    await fetch(`/api/projects/${id}/step/${Math.max(0, newStep)}`, { method: 'PUT' });
    loadProject();
  };
  ...
  const startExecution = () => {
    ...
    const socket = new WebSocket(`${protocol}//${window.location.host}/ws/execute/${id}?auto_continue=${autoContinue}`);
    
    socket.onmessage = (event) => {
        const msg = event.data;
        // If we see the DB update signal, reload project state immediately
        if (msg === '[DB_UPDATE]') {
            loadProject();
            return;
        }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-[calc(100vh-180px)]">
                    ...
                    {isEditing ? (
                        ...
                        <div className="flex-1 space-y-3 overflow-y-auto pr-2 custom-scrollbar">
                            {steps.map(([key, step]: any) => {
                                ...
                                return (
                                    <div key={key} onClick={() => toggleStep(stepNum)} className={`p-4 rounded-lg border transition-all cursor-pointer group ${
                                        isCompleted ? 'bg-slate-700/30 border-emerald-500/30 opacity-60' : 
                                        isCurrent ? 'bg-blue-900/10 border-blue-500/50 shadow-lg shadow-blue-500/5' : 'bg-slate-900 border-slate-700'
                                    }`}>
                                        <div className="flex justify-between items-start gap-3">
                                            <div className="flex items-center gap-2">
                                                {isCompleted ? <CheckCircle2 className="w-4 h-4 text-emerald-500"/> : <Circle className={`w-4 h-4 ${isCurrent ? 'text-blue-500 animate-pulse' : 'text-slate-600 group-hover:text-blue-400'}`}/>}
                                                <h3 className={`font-bold text-sm ${isCurrent ? 'text-blue-400' : isCompleted ? 'text-emerald-400' : 'text-slate-300'}`}>{step.title}</h3>
                                            </div>
                                            {isCurrent && <span className="text-[9px] bg-blue-500/20 text-blue-400 px-1.5 py-0.5 rounded uppercase font-black">Active</span>}
                                        </div>
          ...
          <div className="bg-[#0a0f18] rounded-xl border border-slate-800 flex flex-col h-full min-h-0 shadow-2xl overflow-hidden">
              ...
              <form onSubmit={sendTerminalInput} className="p-3 border-t border-slate-800 bg-slate-900/30 flex gap-2">
